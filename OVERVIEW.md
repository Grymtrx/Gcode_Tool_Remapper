...
| **Exact-match substitution** | Uses regex negative look-around with leading-zero tolerance (`(?<!\d)T0*7(?!\d)`) so `T7` never matches inside `T70` or `T17`, while zero-padded registers like `H01` and `H06` are correctly caught. |
...
3. **Zero-padded H registers are handled correctly** — Haas/Fanuc programs commonly write `H01` or `H06` even for single-digit tools. The pattern `(?<!\d)H0*1(?!\d)` uses `0*` to consume any leading zeros, so `H01`, `H001`, and `H1` all match when remapping tool 1. Boundary safety is preserved: `H010` does not match. This was identified as a critical safety bug (v0.0.3) — a missed H register leaves the tool-length offset wrong, which can cause a crash.
4. **The source file is never overwritten** — the save dialog defaults to `_remapped` suffix, protecting the original.
5. **Pure-function core** makes the remapping logic independently testable without launching the GUI.
6. **Diff preview follows the red/green convention** used by GitHub and major IDEs: changed lines appear as a red "before" row immediately above a green "after" row, making the diff instantly readable to anyone familiar with source control tools. Unchanged lines use a purple highlight on T-number tokens specifically, so the operator can quickly see which tools exist in the file but were not targeted by the current rules — without cluttering H or D registers that are less commonly scanned at a glance.
