...
# Optional encoding detection — falls back gracefully if not installed
try:
    import chardet
    HAS_CHARDET = True
except ImportError:
...
def detect_encoding(path):
    """Best-effort encoding detection. Falls back to latin-1 (safe for G-code)."""
    if not HAS_CHARDET:
        return 'latin-1'
    try:
...
class RemapperApp(tk.Tk):
    def __init__(self):
        ...
        self.rules            = []   # list of (old_num: int, new_num: int)
    ...
    def _browse(self):
        ...
        try:
            ...
            self.status_var.set(
                ...
                f"({line_count} lines, encoding: {enc_note})"
            )
    ...
    def _add_rule(self):
        ...
        for (o, n) in self.rules:
            if o == old_num:
                messagebox.showwarning("Duplicate Rule",
                                       f"A rule for T{old_num} already exists.")
                return

        # Warn on swap — safe with placeholder logic, but good to flag
        existing_sources = {o for (o, n) in self.rules}
        existing_targets = {n for (o, n) in self.rules}
        ...
        self.rules_list.insert(
            "end",
            f"  T{old_num} → T{new_num}    "
            f"(H{old_num}→H{new_num},  D{old_num}→D{new_num})"
        )
    ...
    def _preview(self):
        ...
        for i, (orig, new) in enumerate(zip(orig_lines, new_lines), start=1):
            ...
            if i in changed_set:
                ...
                # Blank separator between change hunks
